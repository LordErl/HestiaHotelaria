...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*itserp\.sh$/)) {
        return true;
      }

      // Allow all itserpapi subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*itserpapi\.duckdns\.org$/)) {
        return true;
      }
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="support@itserp.sh" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="support@itserp.sh" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@itserp.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@itserp.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
